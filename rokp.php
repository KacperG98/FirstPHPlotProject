...
if($res->num_rows > 0){
    ...
    for($i=0;$row = $res->fetch_assoc();$i++){
        $tytul = $row['rok'];
        $table[$i] = array($row['miesiac'].'/'.$row['rok'], $row['min_uzyc'], $row['max_uzyc']);
        $nazwa = $row['nazwa'];
    }
}
...
$plot->SetTitle('Miesiace '.$_COOKIE["dzienod"].'/'.$_COOKIE["miesiacod"].'/'.$_COOKIE["rokod"].'-'.$_COOKIE["dziendo"].'/'.$_COOKIE["miesiacdo"].'/'.$_COOKIE["rokdo"]);
