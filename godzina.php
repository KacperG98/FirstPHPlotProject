...
$req = "select 
            ...
            concat(day(LW.czas),'-',month(LW.czas),'-',year(LW.czas)) as dzien, 
            LW.wartosc as wartosc
        FROM 
            log_wartosci AS LW 
            LEFT JOIN wartosci AS W ON (LW.id_wartosci = W.id)
        where 
            LW.id_wartosci=".$_COOKIE['idw']." 
            AND day(LW.czas)=".$_COOKIE['dzien']." 
            AND month(LW.czas)=".$_COOKIE['miesiac']." 
            AND year(LW.czas)=".$_COOKIE['rok']."
        GROUP BY concat(hour(LW.czas),':', minute(LW.czas))
        ORDER BY LW.czas ASC"; //zapytanie

$table = array(30); //tablica z danymi do tabeli
...
$plot->SetTitle('dzien '.$tytul.' '.$nazwa);
